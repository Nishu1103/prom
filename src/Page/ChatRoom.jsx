...
const ChatRoom = () => {
	...
	const ids = localStorage.getItem("ids");
	const storedData = ids ? JSON.parse(ids) : null;
	const userId = storedData ? storedData.data.id : null;
	const userName = storedData ? storedData.data.name : "Unknown";

	// Redirect if the user or ids is not available
	useEffect(() => {
		if (!userData || !ids) {
			navigate("/");
		}
	}, [navigate, userData, ids]);

	useEffect(() => {
		if (!userId) return;

		socket.current = io("https://lol-2eal.onrender.com");
